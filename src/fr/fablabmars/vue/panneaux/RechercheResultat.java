...
public class RechercheResultat extends ResultPane{
	...
	public RechercheResultat(QueryResult<ArrayList<Utilisateur>> qR){
		super(qR);
	...
	public void update(Observable obs) {
		if(obs instanceof QueryResult<?>){
			try{
				if (((QueryResult<?>)obs).getData() instanceof ArrayList<?>){
					if(((ArrayList<?>)(((QueryResult<?>)obs).getData())).size()==1
							&&((ArrayList<?>)(((QueryResult<?>)obs).getData())).get(0) instanceof Utilisateur){
					
						@SuppressWarnings("unchecked")
						Utilisateur util = new Utilisateur(((ArrayList<Utilisateur>)(((QueryResult<ArrayList<Utilisateur>>)obs).getData())).get(0));
